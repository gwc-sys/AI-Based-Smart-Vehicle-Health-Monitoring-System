...
const unsigned long SOS_DEBOUNCE_MS        = 80;
...
const int DS18B20_PIN        = 4;   // DS18B20 data pin
const int SW420_VIB_PIN      = 33;
const int LDR_LIGHT_PIN      = 34;
const int KY038_SOUND_PIN    = 32;

const int MECHANICAL_BTN_PIN = 27;  // SOS button
...
void scanI2CDevices() {
  ...
  if (count == 0) {
    logWarn("No I2C devices found. MMA7660 may be disconnected.");
  } else {
...
bool setupAccelerometer() {
  logInfo("Initializing MMA7660 on I2C...");
...
bool testTempSensor() {
  ...
  Serial.printf("[TEMP] DS18B20 temp=%.2f\n", t);

  if (t == -999.0) {
    logError("DS18B20 temperature sensor failed");
    return false;
  }

  logPass("DS18B20 temperature sensor OK");
  return true;
}
...
bool testButton() {
  logInfo("Testing SOS button");
  Serial.println("[INFO] Press SOS button within 5 seconds...");

  unsigned long start = millis();
  while (millis() - start < 5000) {
    if (isSOSButtonPressedRaw()) {
      logPass("SOS button OK");
      return true;
    }
    delay(20);
  }

  logWarn("SOS button not pressed during test. Continuing.");
  return true;
}
...
bool runStartupSelfTest() {
  ...
  logInfo("STEP 7: GPS serial");
  r.gpsSerialOk = testGPSSerial();

  logInfo("STEP 8: GPS fix");
  r.gpsFixOk = testGPSFixMandatory();

  logInfo("STEP 9: SOS button");
...
void setup() {
  ...
  pinMode(MECHANICAL_BTN_PIN, INPUT);
...
void loop() {
  ...
  if (now - lastGpsUpdate >= GPS_UPDATE_INTERVAL_MS) {
    ...
    if (now - lastGpsPrint >= 15000) {
      printGPSInfo();
      lastGpsPrint = now;
    }
  }

  SensorData data = updateSensorData();

  isArmed = (data.light < LIGHT_ARM_LEVEL);
  digitalWrite(LED_GRN_PIN, isArmed ? HIGH : LOW);

  // vibration is data-only now, no warning and no LED action

  if (accelReady && data.tiltDetected && isArmed) {
    alarmState = true;
    digitalWrite(RED_LED_PIN, HIGH);

    if (wifiConnected && firebaseReady && (now - lastAlertTime > ALERT_COOLDOWN_MS)) {
      sendAlert("tilt", "Bike tilted or fallen");
      sendSOSAlert("Bike fall detected");
      lastAlertTime = now;
    }
  }

  if (accelReady && data.accidentDetected) {
    alarmState = true;
    digitalWrite(RED_LED_PIN, HIGH);

    if (wifiConnected && firebaseReady && (now - lastAlertTime > ALERT_COOLDOWN_MS)) {
      sendAlert("accident", "Possible accident detected");
      sendSOSAlert("SOS: Accident detected");
      lastAlertTime = now;
    }
  }
  ...
  if (checkSOSButtonPressedEvent()) {
    logInfo("SOS / Reset button pressed");
    triggerSOSAlert("SOS button pressed", wifiConnected);
  }
  ...
  if (now - lastLog >= LOG_INTERVAL_MS) {
    Serial.printf("[DATA] Temp: %.2f C | Light: %d | Vib: %d | Sound: %d\n",
                  data.temperature, data.light, data.vibration, data.sound);
    Serial.printf("[DATA] Armed: %s | Alarm: %s | GPS: %s | Firebase: %s | Sends: %lu\n",
                  isArmed ? "YES" : "NO",
                  alarmState ? "ON" : "OFF",
                  gpsHasFix ? "YES" : "NO",
    ...
    if (accelReady) {
      Serial.printf("[ACCEL] X=%.2f Y=%.2f Z=%.2f G=%.2f Tilt=%s Motion=%s Accident=%s\n",
                    data.accelX, data.accelY, data.accelZ, data.accelTotal,
                    data.tiltDetected ? "YES" : "NO",
                    data.motionDetected ? "YES" : "NO",
                    data.accidentDetected ? "YES" : "NO");
    }
